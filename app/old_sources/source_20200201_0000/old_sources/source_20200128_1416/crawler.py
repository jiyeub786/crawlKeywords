...
def getNaver() :
    ...
    for i,v in enumerate(data):
        ...
        logger.debug("%s\t%s\t%s\t%s\t%s\t%s" % (target_code[0],date,time, str(i+1) , v['keyword'],'https://search.naver.com/search.naver?where=nexearch&query=' +v['keyword'] ))
...
def getDaum() :
    ...
    for i,v in enumerate(searchword_list):
        ...
        logger.debug("%s\t%s\t%s\t%s\t%s\t%s"  % (target_code[1],date, time, str(i+1), v,'https://search.daum.net/search?w=tot&q='+v))
    logger.info('succ pasrsing')
    return datas
...
def getYoutube():
    ...
    for i, v in enumerate(elem_list):
        if 'title' in v.attrs:
            ...
            logger.debug("%s\t%s\t%s\t%s\t%s\t%s" % (target_code[2],date,time,str(i+1), v.attrs['title'], "https://www.youtube.com" + v.attrs['href']))
    logger.info('succ pasrsing')
    return datas
